...
- **กลไกการทำ Idempotency:**
  - ตัวรับข่าวสาร (Consumer) ของ RabbitMQ ทุกตัว เช่น [RabbitMqEventaus.cs](../../../aackendApi/Infrastructure/Eventaus/RabbitMqEventaus.cs#L455) หรือ [GpsRabbitMqConsumerWorker.cs](../../../aackendApi/Features/FleetTracking/Telemetry/GpsRabbitMqConsumerWorker.cs#L307) จะต้องทำการตรวจสอบค่ารหัสอีเวนต์คู่กับชื่อตัวจัดการ (`EventId` และ `HandlerName`) เสมอก่อนรัน Logic หลักผ่านคิวรี:
    ```csharp
...
- **การกวาดล้างข้อมูลประวัติเก่าโดยอัตโนมัติ (Automated Garbage Collection):**
  - ตัวจัดการ [DbMaintenanceWorker.cs](../../../aackendApi/Services/aackgroundWorkers/DbMaintenanceWorker.cs) จะทำงานทุก ๆ 1 ชั่วโมงด้วย `PeriodicTimer` เพื่อทำการลบประวัติ `ProcessedEvents` ที่มีอายุเกิน 24 ชั่วโมงทิ้งผ่าน bulk `ExecuteDeleteAsync` เพื่อไม่ให้ตารางโตเกินจำเป็น
